...
signed main(void)
{
    ...
    vll ans(n - 1, 0);
    map<ll, ll> mp;
    vector<pair<ll, pair<ll, ll>>> pairmp;
    rep(i, n - 1)
    {
        for (ll j = i + 1; j < n; j++)
        {
            ll dis = j - i;
            ll adis = abs(x - i) + abs(y - j) + 1;
            if (i <= x && y <= j)
                dis = dis + 1 - (y - x);
            dis = min(dis, adis);
            pairmp.push_back(make_pair(dis, make_pair(i, j)));
    ...
    {
        mp[i.first]++;
        //debug(i.first, i.second.first, i.second.second);
    }
    for (auto i : mp)
