...
signed main(void)
{
    ll n, m, kach = 0;
    cin >> n >> m;
    vector<tuple<ll, ll, string>> a(2 * n);

    rep(i, 2 * n)
    {
        string hand;
        cin >> hand;
        a[i] = make_tuple(m, i, hand);
        if (i % 2 == 1)
        {
            //debug(get<2>(a[i - 1])[0], get<2>(a[i])[0]);
            kach = judge(get<2>(a[i - 1])[0], get<2>(a[i])[0]);
            if (kach == 1)
                get<0>(a[i - 1])--;
            else if (kach == 2)
                get<0>(a[i])--;
            //debug(get<0>(a[i - 1]), get<0>(a[i]));
        }
    }
    Sort(a);
    ...
    rrep(j, m - 1)
    {
        ...
        rep(i, n)
        {
            //debug(get<2>(a[2 * i])[j], get<2>(a[2 * i + 1])[j]);
            kach = judge(get<2>(a[2 * i])[j], get<2>(a[2 * i + 1])[j]);
            //debug(kach);
            if (kach == 1)
                get<0>(a[2 * i])--;
            else if (kach == 2)
