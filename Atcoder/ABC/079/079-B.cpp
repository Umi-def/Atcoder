...
signed main(void)
{
    ll n;
    cin >> n;
    ld a = (1 + sqrtl(5)) / 2, b = (1 - sqrtl(5)) / 2, ans = 0;
    //debug((1 + sqrtl(5)) / 2, (1 - sqrtl(5)) / 2);
    if (b < 0)
        b *= (-1.0);
    //debug(b);
    if (n % 2 == 0)
        ans = powl(a, n) + powl(b, n);
    else
        ans = powl(a, n) - powl(b, n);
    //debug(powl(a, n), powl(b, n));
    //ld ans = powl(a, n) + powl(b, n);
    int c = (int)(ans * 10.0) % 10;
    //debug(c);
    //debug(ans);
    if (c >= 5)
        ans++;
