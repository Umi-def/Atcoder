...
#define mes(...)                                   \
    ...
    (__VA_ARGS__)
#define mmes_1(a) cout << (a)
#define mmes_2(a, b) cout << (a) << " " << (b)
#define mmes_3(a, b, c) cout << (a) << " " << (b) << " " << (c)
#define mmes(...)                                     \
    CHOOSE3((__VA_ARGS__, mmes_3, mmes_2, mmes_1, ~)) \
    (__VA_ARGS__)
...
ll manl(ll a, ll b, ll x, ll y)
{
    ll length = abs(a - x) + abs(b - y);
    return length;
}
...
signed main(void)
{
    ...
    rep(i, 7)
    {
        ...
        b.erase(0, i);
        //debug(a, b);
        if (s.find(a) + s.find(b) <= s.size() * 2)
        {
            // debug(s.find(a), s.find(b), s.size(), i);
            if (s.find(a) == 0 && s.find(b) == s.size() - (7 - i))
            {
                mes("YES");
                re0;
            }
            if (i == 6 && s.find(a) == 0 && s.find_last_of(b) == s.size() - (7 - i))
            {
                mes("YES");
                re0;
            }
            if (i == 0 || i == 7)
            {
                if (s.find(a) == 0 || s.find(b) == s.size() - 7)
                {
                    mes("YES");
                    re0;
                }
