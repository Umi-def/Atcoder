...
void show_field(_2DArray *map, _2DArray *grids)
{
    ...
    char wall, grid;
    char str[] = " oo+o|++o+-+++++.@";//マップ表示の例を参考にプラス.、自キャラ＠を加えた
    ...
    for (int i = 0; i < w; i++)
    {
        for (int j = 0; j < h; j++)
        {
            ...
            grid = get_2DArray(grids, i, j); //左上から順にグリッドの情報をもらう

            if (grid == 1) //ドットがあるときは配列の16番目.を書くようにする
            {
                bit_or_2DArray(field, i * 2 + 1, j * 2 + 1, 16);
            }
            else if (grid == 2) //自キャラがいる時は配列の17番目＠を書くようにする
            {
    ...
    for (int y = 0; y < h; y++)
    {
        for (int x = 0; x < w; x++)
        {
            printf("%c", str[get_2DArray(field, x, y)]);//左上から取得したマップ情報を表示する
        }
        printf("\n");
    }
}
